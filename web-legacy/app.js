...
import { TerminalView } from "./lib/terminal-view.js";
  ...
  async function api(path, options = {}) {
    ...
    if (state.mode === 'relay' && state.selectedDeviceId) headers["X-Device-Id"] = state.selectedDeviceId;
  ...
  async function boot() {
    try {
      ...
      state.mode = state.user.mode || 'direct';
      if (state.mode === 'direct') {
        state.selectedDeviceId = 'local';
        state.devices = [{ deviceId: 'local', deviceName: '本机', status: 'online' }];
      }
    ...
    app.innerHTML = `
      <section class="manager">
        ...
        <div class="manager-layout">
          <section class="device-section" ${state.mode === 'direct' ? 'hidden' : ''}>
            <h3>设备中心</h3>
