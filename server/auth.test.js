...
test('auth token is stable for the same credentials', () => {
  ...
  assert.equal(auth.authenticated({ headers: { cookie: `${COOKIE_NAME}=${encodeURIComponent(token)}` } }), true);

  const restarted = new AuthManager({ username: 'admin', password: 'secret' });
  assert.equal(restarted.authenticated({ headers: { cookie: `${COOKIE_NAME}=${encodeURIComponent(token)}` } }), true);
});
