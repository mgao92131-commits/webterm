...
const (
	historyRangeContentType = "application/x-protobuf"
	maxHistoryRangeLines    = uint64(256)
)

// parseHistoryRangePath 解析
// GET /api/sessions/{id}/history/range?generation=...&from=...&to=...
func parseHistoryRangePath(method, rawPath string) (sessionID string, generation, from, to uint64, ok bool) {
	...
	if err1 != nil || err2 != nil || err3 != nil || generation == 0 || from == 0 || to < from ||
		to-from >= maxHistoryRangeLines {
		return "", 0, 0, 0, false
	}
	return id, generation, from, to, true
}

func isHistoryRangeRequest(method, path string) bool {
	_, _, _, _, ok := parseHistoryRangePath(method, path)
	return ok
}

func (handler *TransferHTTPHandler) routeHistoryRange(sessionID string, generation, from, to uint64) *HTTPResult {
	terminal, found := handler.sessions.manager.Get(sessionID)
	if !found || terminal == nil || terminal.ScreenRuntime() == nil {
		data := terminalengine.HistoryRangeData{HistoryGeneration: generation}
	...
	data := terminal.ScreenRuntime().HistoryRange(generation, from, to)
	switch data.Status {
	...
	default:
		return handler.historyRangeResult(sessionID, generation, from, to, http.StatusConflict,
			pb.HistoryRangeStatus_HISTORY_RANGE_STATUS_STALE_GENERATION, data, "STALE_GENERATION")
	}
}
