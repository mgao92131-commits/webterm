...
func TestAppRejectsOversizedHTTPProxyRequestBody(t *testing.T) {
	testutil.SkipIfLoopbackListenUnavailable(t)
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	conn, _, err := websocket.Dial(ctx, "ws"+strings.TrimPrefix(server.URL, "http")+"/ws/agent", nil)
	if err != nil {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, server.URL+"/api/sessions", strings.NewReader(strings.Repeat("x", 1<<20+1)))
	if err != nil {
		t.Fatalf("new request: %v", err)
	}
	req.AddCookie(&http.Cookie{Name: relaycore.AuthCookieName, Value: token.Value})
	...
	if streams := app.Streams().Snapshot(); len(streams) != 0 {
		t.Fatalf("oversized request should not create streams: %#v", streams)
	}
}
