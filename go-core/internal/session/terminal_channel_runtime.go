...
func (client *terminalChannelRuntime) writeLatestScreenState(ctx context.Context) bool {
	...
	client.hasScreenData = false
	client.screenMu.Unlock()
	// FrameDeriver is owned exclusively by this writer goroutine. Keep diff work outside
	// screenMu so the terminal actor can replace the latest-value mailbox concurrently.
