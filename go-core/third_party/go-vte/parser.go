...
func (p *Parser) Advance(b byte) {
	if p.state == Utf8State {
		...
		// ECMA-48 C0 controls are bytes, not UTF-8 continuation data. Keep
		// the UTF-8 decoder's partial code point intact while the control
		// state machine executes, then resume it when the control completes.
		p.resumeUTF8 = true
		p.state = GroundState
	}

	p.advanceControl(b)
	if p.resumeUTF8 && p.state == GroundState {
		p.resumeUTF8 = false
		p.state = Utf8State
	}
}
...
func (p *Parser) advanceControl(b byte) {
