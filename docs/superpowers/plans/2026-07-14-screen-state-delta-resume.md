...
实施状态（2026-07-14，分支 feat/screen-delta-resume）：Hello 的完整 resume token
已随 `ScreenClient` 进入 terminal actor，由 actor 在当前权威 state 下完成
cold/instance/epoch/future/barrier/exact/patch 判定。ResumeAck、恢复 Patch 与
Snapshot 共用单一 writer 的容量一 initial-sync slot；writer 实际写成功后才以
完整 state seed `FrameDeriver` 并通知 actor 开放实时 mailbox，在途输出只保留
最新 canonical state，提交后从已写 revision 继续派生。attach 与 Resync 均以
当前 revision 导出，不再推进 canonical revision；重复在途 Resync 被忽略。
恢复慢路径同时编码候选 Patch/Snapshot，并执行 80% 成本降级。
...
实施状态（2026-07-14，分支 feat/screen-delta-resume）：`RemoteTerminalModel`
在完整 mutation 提交后发布不可变 `ProjectionHealth`，`ResumeToken` 只从 model
executor 的同一健康快照产生；结构不完整、引用缺失或 reset 后均回退 cold token。
Android 连接状态已拆为 `CONNECTING → TRANSPORT_CONNECTED → SYNCING → CONNECTED`，
transport 建立后由 runtime 把原子 token 交给 connection 发送 Hello；只有成功应用
Snapshot/Patch 或验证 ResumeAck 后才进入 CONNECTED 并申请交互租约。SYNCING
期间输入与 resize 只缓存不发送；同步超时进入有界 fresh Snapshot resync/reconnect
流程，不会无限循环。
...
实施状态（2026-07-14，分支 feat/screen-delta-resume）：完整 Hello 与 ResumeAck
已在 Task 5 随同步门一并落地；本任务补齐 `first_available_history_line_id` presence
映射、history append 连续性与引用预校验、单调 HistoryTrim，以及 mutation 前捕获
promoted row、先水位后追加、revision 最后提交的原子事务。累计恢复可从 revision 100
直接提交到 150；任何校验失败均保持旧投影，runtime 只建立一个 resync fence，权威
Snapshot 才能解除。对应契约测试已全部启用。
