...
Android 远程访问统一经 Relay，上传只有 Relay HTTP tunnel 这一条数据路径，不提供第二套文件二进制协议。
...
1. **Relay-only 远程路径**：Android 仅走 Relay HTTP tunnel。
...
经 Relay 验证 10 MiB、50 MiB、100 MiB；测试压缩包、图片、无扩展名文件、重名、目录切换、无权限、网络/Relay 断开、用户取消、终端页切换，以及 macOS/Linux/Windows Agent。

## 10. 实施顺序

1. 确定 `fileupload` 与 `filesend` 平行的边界。
2. 新建 Agent `fileupload.Service`、CWD 快照和文件系统单元测试。
3. 将 SessionRouter 升级为通用流式 HTTP 请求入口，同时保持 filesend 回归通过。
...
8. 完成 Relay、大文件、弱网、通知并存和跨平台验证。

完成条件：Android 选取单个文件后，文件仅保存到 Agent 在上传开始时固定的 `liveCwd/WebTermUploads`；Relay 请求由 Agent 文件上传服务处理；传输不随文件大小线性占用内存；取消或失败不留下完整名残缺文件；用户能在终端页及传输通知中看到方向正确的进度和结果。
