...
说明：cursor 场景历史为空，旧路径已非常廉价，新路径因每帧光标移动触发旧+新两行重转，整体接近持平；其他场景 ExportState 成本下降 3.7–6.0 倍。历史窗口重导出是最大收益来源。
...
history-heavy 已与 screen-only 收敛到同一量级，历史窗口增量导出目标达成。
...
| 单行更新的 projection 分配量显著下降 | ✅ cursor/tui/scroll 等场景 B/op 下降 2.5–3 倍以上 |
...
阶段 2 全部目标达成。下一阶段（阶段 3）将按修订后计划 §7.0 先修复 Android 跨边界选择复制 bug，再实施 `TerminalHistory` 分段不可变模型迁移。
