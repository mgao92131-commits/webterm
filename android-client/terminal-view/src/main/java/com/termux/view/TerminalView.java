...
public final class TerminalView extends View {
    ...
    public void onScreenUpdated(boolean skipScrolling) {
        ...
        boolean autoScrollDisabled = mEmulator.isAutoScrollDisabled();
        boolean scrolledAwayFromBottom = mTopRow != 0;

        if (selectingText || autoScrollDisabled || scrolledAwayFromBottom) {

            // Do not follow new output while selecting text, auto-scroll is disabled,
            // or the user has manually scrolled away from the bottom.
            int rowShift = mEmulator.getScrollCounter();
            if (-mTopRow + rowShift > rowsInHistory) {
                // If selected text is pushed out of history, abort text selection
                // and allow the view to follow new output to the bottom.
                if (selectingText)
                    stopTextSelectionMode();

                // For explicit or gesture-driven scrollback holds, keep showing the
                // oldest available history instead of jumping to the bottom.
                if (autoScrollDisabled || (!selectingText && scrolledAwayFromBottom)) {
    ...
    public void updateSize() {
        ...
        if (mEmulator == null || (newColumns != mEmulator.mColumns || newRows != mEmulator.mRows)) {
            boolean wasScrolledToBottom = mTopRow == 0;
            int scrollOffset = mEmulator == null ? 0 : mEmulator.getScreen().getActiveTranscriptRows() + mTopRow;
            ...
            if (wasScrolledToBottom) {
                ...
                int rowsInHistory = mEmulator.getScreen().getActiveTranscriptRows();
                mTopRow = Math.min(0, Math.max(-rowsInHistory, scrollOffset - rowsInHistory));
            }
            scrollTo(0, 0);
            invalidate();
        }
