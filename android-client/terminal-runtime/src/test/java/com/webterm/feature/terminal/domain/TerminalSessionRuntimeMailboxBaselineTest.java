...
public final class TerminalSessionRuntimeMailboxBaselineTest {
  ...
  public void mailboxBurst_baseline() throws Exception {
    ...
    Field mailboxField = TerminalSessionRuntime.class.getDeclaredField("screenMailbox");
    mailboxField.setAccessible(true);
    ScreenMailbox mailbox = (ScreenMailbox) mailboxField.get(runtime);
    ...
    for (int revision = 1; revision <= BURST_FRAMES; revision++) {
      ...
      maxDepth = Math.max(maxDepth, mailbox.pendingMessages());
    }
    ...
    assertTrue("mailbox is empty after drain", mailbox.pendingMessages() == 0);
