...
public final class RemoteTerminalSnapshotConsistencyTest {
  ...
  public void anchorRestoreUsesConsumedRenderSnapshot() {
    ...
    assertTrue(applyHistoryCommit(model, 1, 2,
        new HistoryExtent(1, 100), createHistoryLines(51, 100)));
  ...
  public void modelAdvanceAfterConsumeDoesNotChangeAnchorGeometry() {
    ...
    assertTrue(applyHistoryCommit(model, 1, 2,
        new HistoryExtent(1, 100), createHistoryLines(31, 100)));
  ...
  public void scrollBoundsMatchRenderedSnapshot() {
    ...
    assertTrue(applyHistoryCommit(model, 1, 2,
        new HistoryExtent(1, 100), createHistoryLines(21, 100)));
  ...
  private static boolean applyHistoryCommit(RemoteTerminalModel model,
                                            long baseRevision, long revision,
                                            HistoryExtent extent,
                                            List<TerminalLine> lines) {
    try {
      return model.applyTerminalCommit(new TerminalCommit(
          "i1", 1, 1, baseRevision, revision, null,
          new HistoryMutation(extent, lines), null, null, null));
