 ...
 */
class EmojiPresentationClassifier {
  ...
  private static final int[] DEFAULT_EMOJI_RANGES = {
      0x231A, 0x231B,
      0x23E9, 0x23F3,
      0x25FD, 0x25FE,
      0x2614, 0x2615,
  ...
  boolean isEmojiPresentation(String grapheme) {
    ...
    // 单个 ASCII code point 不可能是默认 Emoji presentation。不能用首 code point
    // 拒绝整个 grapheme，因为 1️⃣、#️⃣、*️⃣ 的首字符本身是 ASCII。
    if (grapheme.length() == 1 && grapheme.charAt(0) < 0x80) return false;
  ...
  private static boolean isDefaultEmojiPresentation(int codePoint) {
    ...
    return inRanges(codePoint, DEFAULT_EMOJI_RANGES);
  }
